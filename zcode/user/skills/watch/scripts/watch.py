...
from funasr_asr import transcribe_video as funasr_transcribe  # noqa: E402
from funasr_asr import load_model as funasr_load_model  # noqa: E402
from whisper import load_api_key, transcribe_video as whisper_transcribe  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--asr",
        choices=["funasr", "whisper", "auto"],
        default="auto",
        help="Speech recognition backend. 'funasr' uses local FunASR (SenseVoiceSmall), "
             "'whisper' uses Groq/OpenAI Whisper API. 'auto' tries funasr first, falls back to whisper. "
             "Default: auto.",
    )
    ap.add_argument(
        "--no-dedup",
    ...
    # ── ASR (speech recognition) back-end ──────────────────────────────────────
    # Priority:
    #   --asr funasr → uses local FunASR only
    #   --asr whisper → uses Whisper API (Groq/OpenAI) only
    #   --asr auto (default) → tries FunASR first, falls back to Whisper
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        funasr_model_instance = None
        transcription_ok = False

        # Try FunASR (local) first
        if args.asr in ("funasr", "auto"):
            try:
                if funasr_model_instance is None:
                    funasr_model_instance = funasr_load_model()
                all_segments, used_backend = funasr_transcribe(
                    video_path,
                    work / "audio.mp3",
                    model=funasr_model_instance,
                )
                transcript_segments = filter_range(all_segments, start_sec, end_sec) if focused else all_segments
                transcript_text = format_transcript(transcript_segments)
                transcript_source = used_backend
                transcription_ok = True
                print(f"[watch] transcribed via {transcript_source}", file=sys.stderr)
            except SystemExit as exc:
                print(f"[watch] FunASR transcription failed: {exc}", file=sys.stderr)
                if args.asr == "funasr":
                    print("[watch] └─ re-run with --asr whisper or --asr auto to fall back", file=sys.stderr)

        # Fallback to Whisper API
        if not transcription_ok and args.asr in ("whisper", "auto"):
            whisper_backend, whisper_api_key = load_api_key(args.whisper)
            if whisper_backend and whisper_api_key:
                try:
                    all_segments, used_backend = whisper_transcribe(
                        video_path,
                        work / "audio.mp3",
        ...
        if not transcription_ok:
            print("[watch] no ASR backend succeeded — proceeding without transcription", file=sys.stderr)
