...
description: >-
  Create and test cross-platform agent skills from workflow descriptions.
  Activates when users ask to create an agent, automate a repetitive workflow,
  create a custom skill, test a skill, verify skill quality, or need advanced
  agent creation. Triggers on phrases like create agent for, automate workflow,
  create skill for, every day I have to, daily I need to, turn process into
  agent, need to automate, create a cross-platform skill, validate this skill,
  export this skill, migrate this skill, test this skill, TDD for skills,
  pressure test skill, verify skill works. Supports single skills, multi-agent
  suites, transcript processing, template-based creation, interactive
  configuration, cross-platform export, spec validation, and TDD-based quality
  assurance.
license: MIT
metadata:
...
## TDD for Skills — Quality Assurance
...
| Phase | Action |
|-------|--------|
| **RED** | Run pressure scenario WITHOUT skill — document baseline behavior and rationalizations |
...
|------|-----------|-----------------|
| **Discipline-enforcing** (rules) | Pressure scenarios: time + sunk cost + exhaustion combined | Agent follows rule under maximum pressure |
| **Technique** (how-to) | Application + variation + missing info scenarios | Agent applies technique correctly to new scenario |
| **Pattern** (mental model) | Recognition + counter-example scenarios | Agent correctly identifies when/how to apply |
| **Reference** (docs/APIs) | Retrieval + gap testing | Agent finds and correctly applies information |

### Claude Search Optimization (CSO)

The `description` field is the primary activation mechanism. Future Claude reads it to decide which skills to load.

**Rules:**
- Start with "Use when..." — focus on triggering conditions ONLY
- **NEVER summarize the skill's workflow** in the description (Claude will follow the summary instead of reading the skill)
- Include error messages, symptoms, and tools as keywords
- Write in third person, max 1024 chars
...
Agents under pressure rationalize skipping rules. Counter explicitly:
...
Add rationalization tables and red-flags lists directly in discipline-enforcing skills.

See `references/testing-skills-with-subagents.md` for the complete testing methodology.
See `references/anthropic-best-practices.md` for Anthropic's official skill authoring guidance.
See `references/persuasion-principles.md` for the psychology behind rationalization defense.
...
| `references/testing-skills-with-subagents.md` | Complete TDD testing methodology for skills |
| `references/anthropic-best-practices.md` | Anthropic's official skill authoring best practices |
| `references/persuasion-principles.md` | Psychology of rationalization defense (Cialdini, 2021) |
| `references/graphviz-conventions.dot` | Graphviz style rules for skill flowcharts |
| `references/render-graphs.js` | Tool to render skill flowcharts to SVG |
| `references/examples/CLAUDE_MD_TESTING.md` | Example: testing CLAUDE.md configurations |
